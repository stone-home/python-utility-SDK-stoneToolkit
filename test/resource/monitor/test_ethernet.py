...
class TestEthernetMonitor:
    ...
    def test_get_structured_data(self, mock_file, instance, input_data):
        with open(mock_file) as f:
            ...
            with patch.object(EthernetMonitor, "_get_raw_data", return_value=_data):
                ...
                with patch(
                    f"stone_lib.resource.monitor.ethernet.EthernetMetrics",
                    side_effect=[mock_class_1, mock_class_2],
                ) as mock_class:
                    result = instance._get_structured_data()
                    assert result == {"lo": mock_class_1, "eth0": mock_class_2}
                    for line in _data[2:]:
                        if line.strip() == "":
                            continue
    ...
    @patch.object(
        ...
        return_value={"lo": MagicMock(), "eth0": MagicMock()},
    )
    ...
    def test_get_all_interfaces(self, mock_get_structured_data, instance):
        assert (
            instance.get_all_interfaces_data() == mock_get_structured_data.return_value
        )
    ...
    def test_get_specific_interface_data(self, mock_get_structured_data, instance):
        assert (
            instance.get_specific_interface_data("lo")
            == mock_get_structured_data.return_value["lo"]
        )
        assert (
            instance.get_specific_interface_data("eth0")
            == mock_get_structured_data.return_value["eth0"]
        )
    ...
    def test_to_json(self, mock_get_structured_data, instance):
        assert instance.to_json() == {
            interface: mock_get_structured_data.return_value[interface].to_json()
